...
def sample_gt(gt, train_size, dataset_name, mode='random'):
    ...
    if mode == 'random':
        # train_indices, test_indices = sklearn.model_selection.train_test_split(X, train_size=train_size, random_state=0, stratify=y)
        ...
        for k, v in data_pos.items():
            if train_size > 0 and train_size < 1:
                train_num = train_size * len(v)
                train_num = int(train_num)
            elif len(v) < train_size:
                train_num = 15
            else:
            ...
            # np.random.seed(0)
            # num_list = np.random.randint(len(v), size=train_num)
            random.seed(0)
            ll = len(v)
            num_list = random.sample(range(ll), train_num)
            train_pos[k] = [v[i] for i in num_list]
            # # train_pos[k] = random.sample(v, int(train_num))
            # # test_pos[k] = [i for i in v if i not in train_pos[k]]
            test_pos[k] = [i for i in v]
        ...
        for i in range(1, class_num+1):
            ...
            for j in range(0, len_temp):
                temp_d = temp[j]
                train_indices.append((temp_d[0], temp_d[1]))
                ii = temp_d[0]
                jj = temp_d[1]
                train_gt[ii][jj] = i
        ...
        for i in range(1, class_num+1):
            ...
            for j in range(0, len_temp):
                temp_d = temp[j]
                test_indices.append((temp_d[0], temp_d[1]))
                ii = temp_d[0]
                jj = temp_d[1]
                test_gt[ii][jj] = i
